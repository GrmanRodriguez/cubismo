...
def main(args):
    ...
    train_loader = DataLoader(dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.loader_num_workers, collate_fn=collate_by_gender)

    print(f"Setting up training params ({args.epochs=}, {args.lr=})")
    optimizer = torch.optim.Adam(policy.parameters(), lr=args.lr)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs)
    
    checkpoints_base_name = f"celeba_{time.time():.0f}"
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % args.eval_and_checkpoint_every == 0 or (epoch + 1) == args.epochs:
            ...
            evaluation_name = f"{evaluations_directory}/epoch={epoch+1}_loss={loss.item():.6f}.jpg"
